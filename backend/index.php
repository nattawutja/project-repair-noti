...
$data = [];
// ทดสอบ query ง่าย ๆ
$qry = 'select concat(t5."firstName",\' \',t5."lastName") as fullnameit,t3."name" as dviname,t4."name",CASE WHEN t1."StatusWork" = 0 THEN \'รอ IT ตรวจสอบ\' WHEN t1."StatusWork" = 1 THEN \'กำลังดำเนินการ\' WHEN t1."StatusWork" = 2 THEN \'รอผู้แจ้งตรวจสอบ\' else \'จบงาน\' end as status, CASE WHEN t1."SystemType" = \'P\' THEN \'P/C\' else \'AS/400\' end as systemname,t2."name_Device",to_char(t1."create_date",\'DD/MM/YYYY HH24:MI:SS\')as cvcreatedate,t1.* from "rp_Repair_Notify" t1
